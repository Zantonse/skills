...
description: Parse ~/.claude/ session logs to produce data-driven usage reports with token consumption, tool-chain workflow analysis, skill gap detection, and personalized recommendations. This skill runs a collector script that reads JSONL session transcripts (including subagent logs), computes tool-call bigram/trigram sequences, extracts token metrics and cache hit rates, and identifies repetitive patterns — data that cannot be obtained without parsing the log files directly. MUST use this skill for any request involving Claude Code usage analysis, session history review, token efficiency audits, skill usage audits, workflow pattern detection, session health checks, or recommendations for which skills to use, create, or install. Trigger on any mention of: usage patterns, session analysis, token waste, compaction frequency, tool workflows, skill recommendations, "how am I using Claude", "what should I automate", "analyze my sessions", "which skills am I not using", "where am I wasting tokens", "session health", "usage breakdown", "usage report", or questions about how to get more out of Claude Code. Also trigger when the user mentions burning tokens, context getting compacted too often, wanting to understand their patterns, or wanting to track usage trends over time. Do NOT confuse with: account research (researching a company), transcript analysis (analyzing a call recording), CLAUDE.md optimization (editing config files), skill creation (building new skills), or vault organization (sorting Obsidian notes).
---
...
Read the progress output to confirm it ran successfully, then read the JSON data file. If `~/.claude/history.jsonl` does not exist, tell the user: "No session history found yet — history accumulates as you use Claude Code. Run this skill again after a few sessions."
