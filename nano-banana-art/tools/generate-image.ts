...
/**
 * Parse a single env file line, handling optional "export " prefix and quotes.
 */
function parseEnvLine(line: string): [string, string] | null {
  let trimmed = line.trim();
  if (!trimmed || trimmed.startsWith('#')) return null;
  // Strip "export " prefix (used in ~/.claude-litellm.env)
 ...
 * Reads ~/.claude-litellm.env first (LiteLLM credentials),
 * then ~/.claude/.env (Google API key, etc.).
 * Shell env vars always take precedence.
 */
async function loadEnv(): Promise<void> {
  const envFiles = [
    resolve(process.env.HOME!, '.claude-litellm.env'),
  ...
  for (const envPath of envFiles) {
    try {
      ...
      for (const line of envContent.split('\n')) {
        ...
        // Only set if not already defined (allow overrides from shell or earlier files)
        if (!process.env[key]) {
...
type Provider = "google" | "openrouter" | "litellm";
...
const LITELLM_MODELS: Record<Model, string> = {
  "nano-banana-2": "gemini-3.1-flash-image-preview",
  "nano-banana-pro": "gemini-3-pro-image-preview",
};

const LITELLM_SIZE_MAP: Record<GeminiSize, string> = {
  "512px": "512x512",
  "1K": "1024x1024",
  "2K": "2048x2048",
  "4K": "4096x4096",
...
MORE INFO:
  Documentation: ~/.claude/skills/nano-banana-art/SKILL.md
  Aesthetic: ~/.claude/skills/nano-banana-art/aesthetic.md
`);
  process.exit(0);
}
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (key) {
      case "model":
        ...
        break;
      case "provider":
        if (value !== "google" && value !== "openrouter" && value !== "litellm") {
          throw new CLIError(`Invalid provider: ${value}. Must be: google, openrouter, or litellm`);
        }
...
async function generateWithLiteLLM(
  ...
  if (!apiKey || !baseUrl) {
    throw new CLIError("Missing LITELLM_API_KEY or LITELLM_BASE_URL in ~/.claude-litellm.env");
  }

  const litellmModel = LITELLM_MODELS[model];
  const litellmSize = LITELLM_SIZE_MAP[size];
  const modelLabel = model === "nano-banana-pro" ? "Nano Banana Pro" : "Nano Banana 2";
  console.log(`Generating with ${modelLabel} via LiteLLM (${litellmModel}) at ${size} ${aspectRatio}...`);

  // Append aspect ratio hint to prompt (LiteLLM images endpoint doesn't have native aspect ratio)
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new CLIError(`LiteLLM API error: ${response.status} - ${errorText}`);
  }

  const data = await response.json() as any;

  if (!data.data || !data.data[0] || !data.data[0].b64_json) {
    throw new CLIError("No image data returned from LiteLLM. The prompt may have been blocked by safety filters.");
  }

  const imageBuffer = Buffer.from(data.data[0].b64_json, "base64");
  if (imageBuffer.length === 0) {
    throw new CLIError("LiteLLM returned empty image data. The prompt may have been silently filtered.");
  }
...
async function main(): Promise<void> {
  try {
    ...
    // Priority: litellm > google > openrouter
    let provider: Provider;
    if (args.provider) {
      provider = args.provider;
    } else if (process.env.LITELLM_API_KEY && process.env.LITELLM_BASE_URL) {
      provider = "litellm";
      ...
      throw new CLIError("No API key found. Set LITELLM_API_KEY+LITELLM_BASE_URL in ~/.claude-litellm.env, or GOOGLE_API_KEY in ~/.claude/.env");
    }

    // Warn about provider-specific features when using non-Google providers
    if (provider === "openrouter" || provider === "litellm") {
      if (args.thinking) {
        console.warn(`Warning: --thinking is not supported with ${provider} provider, ignoring`);
      }
      if (args.grounded) {
        console.warn(`Warning: --grounded is not supported with ${provider} provider, ignoring`);
      }
      if (args.referenceImage) {
        console.warn(`Warning: --reference-image is not supported with ${provider} provider, ignoring`);
      }
    }
    ...
    const generate = (prompt: string, output: string): Promise<void> => {
      if (provider === "litellm") {
        return generateWithLiteLLM(args.model, prompt, args.size, args.aspectRatio, output);
      }
